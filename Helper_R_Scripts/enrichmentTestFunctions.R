...
enricherOnGroups <- function(groupTable, geneColumn = "gene", groupColumns = c("group"),
  ...
  if (Sys.info()["sysname"] == "Windows"){
    ...
    clusterExport(cl, list( "setDT", "universe", "term2gene.gmt", "geneColumn"), envir = environment())
...
#' enricherOnGroups, simplifyEnrichBySimilarUniverseMembership, enrichHeatmapBestPerGroup
#' 
#' groupTable - dt with 2 columns, Gene, Experimental Group
#' geneColumn - "", name of Gene column 
#' gmt - gene ontology dt
#' 

enrichmentOnGroupsPL <- function (groupTable, geneColumn = "Protein", groupColumns = "Group ", gmt = "gmt",universe = NULL, numProcessors = 1,
